Hi, it's me Skie 109, I made a game in Python called Apple Picking Simulator

What is this game about:
The game involves picking apples from the tree,
and then selling them for coins, which are used to buy baskets, as the first basket holds a maximum of four apples.
Your goal is to buy the largest basket to win the game.
The game was created in Poland

What I want to add in v.1.1:
English language option, changing biomes

Have a nice game :P

-------------For Polish (Dla Polaków)-------------

Cześć, to ja, Skie 109, stworzyłem grę w Pajtonie o nazwie Symulator zbierania jabłek.

O czym jest ta gra:
Gra polega na zbieraniu jabłek z drzewa,
a następnie sprzedawaniu ich za monety, za które kupuje się koszyki, ponieważ pierwszy koszyk mieści maksymalnie cztery jabłka.
Twoim celem jest kupienie największego koszyka, aby wygrać grę.
Gra powstała w Polsce.

Co chcę dodać w wersji 1.1:
Wybór języka angielskiego
Zmieniające się biomy i nowe koszyki

Miłej gry :P
--------------------------
Uwaga: Chociaż gra jest dostępna na licencji open source, jej kopiowanie jest niechciane. Dziękujemy za zrozumienie.
Note: Although the game is available under an open source license, copying it without credit is discouraged. Thank you for your understanding.
